function propertyNameRequiresQuotes(propertyName: string) {
    try {
        ...
        new Function(
            'ctx',
            `ctx.worksWithoutQuotes = {${propertyName}: true}['${propertyName}']`,
        )();
    ...
    switch (typeof value) {
        ...
        case 'object':
            ...
            if (value instanceof Date) {
                return `${indentString.repeat(
                    indentLevel,
                )}new Date(${quoteString(value.toISOString(), {
                    doubleQuote,
                })})`;
            }
            ...
            value = Object.keys(value).reduce<string[]>(
                (entries, propertyName) => {
                    ...
                    if (propertyValueString) {
                        const quotedPropertyName = propertyNameRequiresQuotes(
                            propertyName,
                        )
                        ...
                        if (
                            ...
                            entries.push(
                                `${indentString.repeat(
                                    indentLevel + 1,
                                )}${quotedPropertyName} ${trimmedPropertyValueString.slice(
                                    propertyName.length,
                                )}`,
                            );
                        }
                            ...
                            entries.push(
                                `${indentString.repeat(
                                    indentLevel + 1,
                                )}${quotedPropertyName}: ${trimmedPropertyValueString}`,
                            );
                        }
                    }
        ...
        case 'symbol': {
            ...
            if (key) {
                return `${indentString.repeat(indentLevel)}Symbol(${quoteString(
                    key,
                    {
                        doubleQuote,
                    },
